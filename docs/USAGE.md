...
| `gc [--dry-run] [--yes]` | Find and destroy orphaned cloud resources, and summarize ongoing cost. Classifies every listed resource three ways: **orphan** (dispatcher-owned, its run is gone) is reaped; **standing** (dispatcher-owned, tied to no run — e.g. a driver-baked image) is listed and kept; **external** (not dispatcher-owned) is listed only and never touched. Reaping only ever acts on resources tagged `dispatcher=true`. On GCP the audit covers instances, disks, images, snapshots, and static IPs with an estimated `~$/mo`; other providers currently list instances only. Prompts before destroying; `--dry-run` previews, `--yes`/`-y` skips the prompt. |
