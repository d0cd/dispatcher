...
| ✅ **Shard fan-out wired (count mode)** — `runRun` branches to `runSharded`; each shard is a full dispatcher run with `SHARD_INDEX`/`SHARD_COUNT` injected via `WorkloadSpec.Env` (threaded through the 4 secret-handling env helpers). Approval isn't bypassed. **Verified end-to-end over local-process.** | L | High |
| **Shard refinements** — (1) discover-mode item delivery via `SHARD_ITEMS_FILE` (local: temp path; docker/cloud need per-shard staging); (2) output aggregation (merge each shard-run's artifacts into one dir per `aggregate.outputs`); (3) LPT scheduling — reorder assignments by `cost/history.go` durations (engine currently runs in index order). | M | Medium |
