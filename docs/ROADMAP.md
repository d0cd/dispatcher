...
| **Structured failure evidence before cleanup** — *delivered (classification half):* wrapped signal exits (137/247 = SIGKILL, 143/241 = SIGTERM) now classify transient instead of permanent-application-error; crash signals stay permanent. Still open: capture guest OOM/cgroup state, container inspect data, and a bounded kernel tail *before* removing containers or VMs, preserving uncertainty when evidence is absent. | M | High |
