...
- **Attestation.** A TEE encrypts memory, but the *proof* is a signed attestation report, and the live evidence path is implemented on all three clouds. An in-TEE measured agent binds a per-run nonce and its channel key into the evidence (`REPORT_DATA = H(nonce‖channel-key)`), returns the report/token, and dispatcher verifies it before the workload runs: **GCP** Confidential Space (measured agent-image digest + Google JWS), **Azure** MAA (JWKS fetched and pinned from the instance) plus a measured direct-SNP path, and **AWS** SEV-SNP (`VLEK→ASVK→ARK` chain with CRL) plus Nitro Enclaves (COSE → pinned Root-G1 + PCR0). `attestation: required` (the default) runs this verified path; `attestation: off` remains the explicit escape hatch for **encrypted-memory-without-verification** (data-in-use protection, no proof the TEE is genuine).
- **Attestation — honest residual (agent measurement).** On the *standard* AWS SEV-SNP and Azure MAA paths the agent is delivered over SSH and is **not** folded into the launch measurement, so those attest the firmware/CVM image but not the agent binary (see the SECURITY NOTE in `confidential_aws.go`). The **measured backends close this**: GCP Confidential Space (the agent image digest *is* the measurement), Azure `confidential.profile: azure-snp` (agent in a dm-verity root → PCR11), and AWS `confidential.profile: nitro` (enclave PCR0). Operator setup: supply the cross-compiled agent binary or a pinned measured image via the `DISPATCHER_*` env vars or the `dispatcher confidential` pin registry.
