...
- **Attestation.** A TEE encrypts memory, but the *proof* is a signed attestation report, and the live evidence path is implemented on all three clouds. An in-TEE measured agent binds a per-run nonce and its channel key into the evidence (`REPORT_DATA = H(nonce‖channel-key)`), returns the report/token, and dispatcher verifies it before the workload runs: **GCP** Confidential Space (measured agent-image digest + Google JWS), **Azure** MAA (JWKS fetched and pinned from the instance) plus a measured direct-SNP path, and **AWS** SEV-SNP (`VLEK→ASK→ARK` chain with CRL) plus Nitro Enclaves (COSE → pinned Root-G1 + PCR0). `attestation: required` (the default) runs this verified path; `attestation: off` remains the explicit escape hatch for **encrypted-memory-without-verification** (data-in-use protection, no proof the TEE is genuine).
