...
- **GPU workloads**: detection → feasibility → catalog/pricing → provisioning. GCP/AWS provision GPU instances from an operator driver-baked image (`DISPATCHER_{GCP,AWS}_GPU_IMAGE`); the code path is covered by tests. A repeatable end-to-end live validation (provision → `nvidia-smi`) is pending — hyperscaler GPU quota starts at 0, so this is gated on a quota grant or a quota-free GPU provider (e.g. Lambda). k8s uses `nvidia.com/gpu` limits.
