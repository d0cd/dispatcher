...
- **GPU workloads**: detection → feasibility → catalog/pricing → provisioning. **Live-validated end-to-end on Lambda** (`lambda-vm`): provision a real GPU box → `nvidia-smi` in-VM → teardown, zero residual. GCP/AWS provision GPU instances from an operator driver-baked image (`DISPATCHER_{GCP,AWS}_GPU_IMAGE`); that path is test-covered but its live validation is pending GPU quota (hyperscaler quota starts at 0). k8s uses `nvidia.com/gpu` limits.
