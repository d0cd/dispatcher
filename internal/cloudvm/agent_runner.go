...
func defaultRunner(ctx context.Context, p runPayload) runResult {
	if len(p.Command) == 0 {
		return runResult{ExitCode: -1, Stderr: []byte("empty command")}
	}
	dir, err := os.MkdirTemp("", "dispatcher-workload-*")
	if err != nil {
		return runResult{ExitCode: -1, Stderr: []byte("workdir: " + err.Error())}
	}
	defer os.RemoveAll(dir)

	if len(p.SourceTarGz) > 0 {
		if err := unTarGz(p.SourceTarGz, dir); err != nil {
			return runResult{ExitCode: -1, Stderr: []byte("extract source: " + err.Error())}
		}
	}
	...
	res := runResult{ExitCode: code, Stdout: stdout.Bytes(), Stderr: stderr.Bytes()}
	if len(p.Outputs) > 0 {
