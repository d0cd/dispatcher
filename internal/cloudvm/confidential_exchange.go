...
// runPayload is what dispatcher seals to the agent's channel key and POSTs to
...
// runResult is what the agent seals back (to runPayload.ResultPubKey) and serves
...
func csEndpointFetch(baseURL string) csFetch {
	return func(ctx context.Context, _ *VMInfo, _, _ string, nonce []byte) (csEvidence, error) {
...
func runSealedExchange(ctx context.Context, baseURL string, channelKey []byte, payload runPayload) (runResult, error) {
	...
	if err != nil {
		return runResult{}, fmt.Errorf("seal payload: %w", err)
	}
	...
	if err != nil {
		return runResult{}, fmt.Errorf("post payload: %w", err)
	}
	...
	if postResp.StatusCode != http.StatusAccepted && postResp.StatusCode != http.StatusOK {
		return runResult{}, fmt.Errorf("agent rejected payload: %d: %s", postResp.StatusCode, bytes.TrimSpace(body))
	}
...
func pollSealedResult(ctx context.Context, baseURL string, resultPriv []byte) (runResult, error) {
	for {
		...
		resp, err := csHTTPClient().Do(req)
		if err != nil {
			return runResult{}, fmt.Errorf("poll result: %w", err)
		}
		...
		case http.StatusOK:
			...
			if err != nil {
				return runResult{}, fmt.Errorf("open sealed result: %w", err)
			}
			var r runResult
			if err := json.Unmarshal(plain, &r); err != nil {
				return runResult{}, fmt.Errorf("parse result: %w", err)
			}
			return r, nil
		...
		default:
			return runResult{}, fmt.Errorf("result poll failed: %d: %s", resp.StatusCode, bytes.TrimSpace(body))
		}
	}
}
