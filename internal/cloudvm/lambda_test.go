...
func TestNewLambdaProvider_ResolvesKeyFromSecretCommand(t *testing.T) {
	...
	secrets.SetGlobal(map[string][]string{"DISPATCHER_LAMBDA_API_KEY": {"printf", "resolved-key"}})
	t.Cleanup(func() { secrets.SetGlobal(nil); os.Unsetenv("DISPATCHER_LAMBDA_API_KEY") })
